  ...
  return (
    <div className="flex flex-col items-center">
      ...
      {node.children && node.children.length > 0 && (
        <div className="mt-8 flex justify-center gap-8 relative">
          {/* Vertical line from parent */}
          <div className="absolute top-[-2rem] h-8 w-px bg-muted-foreground/30" />
          {node.children.map(child => (
            <div key={child.id} className="relative flex flex-col items-center">
              {/* Horizontal line to child */}
              <div className="absolute top-[-2rem] left-1/2 h-px w-full bg-muted-foreground/30" />
               <div className="absolute top-[-2rem] left-0 h-8 w-px bg-muted-foreground/30" />
