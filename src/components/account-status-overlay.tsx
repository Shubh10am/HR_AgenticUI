...
import { ShieldAlert, PauseCircle, LogOut, Mail, Phone, AlertCircle } from 'lucide-react';
...
const statusDetails = {
  Hold: {
    ...
    reason: "This is often due to a pending verification, a billing issue, or a routine review. Full access to the platform's features is currently restricted until the issue is resolved.",
  },
  Suspended: {
    icon: ShieldAlert,
    title: 'Account Suspended',
    description: "Your organization's account has been suspended.",
    reason: "This is typically due to a significant issue such as a violation of our terms of service. Access to the platform has been disabled pending a review.",
  },
};
...
export default function AccountStatusOverlay({ status }: AccountStatusOverlayProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-background/80 backdrop-blur-sm p-4">
      <Card className="w-full max-w-lg shadow-2xl animate-fade-in flex flex-col max-h-[90vh]">
        ...
        <ScrollArea className="flex-grow min-h-0">
            ...
            <div className="bg-secondary/50 p-4 rounded-lg my-6 text-left">
                    <h4 className="font-semibold mb-2 text-foreground">Common Reasons for Account Review:</h4>
                    <ul className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-1 list-disc list-inside space-y-0 text-muted-foreground text-xs">
                        {commonReasons.map((reason, index) => <li key={index}>{reason}</li>)}
                    </ul>
                </div>
                <p className="text-center text-muted-foreground mb-6">
                    Please contact your organization's administrator. If you are the administrator, please reach out to our support team to resolve this issue.
                </p>
